...
## 26. [배포 후 실측 라운드] ①~⑨ 전 항목 실측 완료 + 멀티턴 버그 수정 다수

MEASUREMENT_GUIDE.md 절차대로 성능/비용/품질 지표 9개 항목을 전부 배포 서버에서
실측했다. 그 과정에서 실제 대화 시나리오(멀티턴, 재고/가격 질문)를 반복
재현·수정하며 여러 실사용 버그를 같이 잡았다. 상세 수치는 PRESENTATION_SCRIPT.md
11절, PHASE3_BENCHMARK.md 참고.

### 실측 완료 (①~⑨)
①Provider 4종(OpenAI/Gemini/Anthropic/DeepSeek) 비교, ②RAGAs CI 게이트
성공/실패 둘 다 실제 PR로 검증, ③LangSmith Router vs Agent 트레이스 비교,
④하이브리드 검색 4구성(A~D) ablation, ⑤개인화 A/B, ⑥환각 가드 실측(4개
provider halluc_bait 전부 0%), ⑦재고 락 부하테스트(성공10/거절40/초과판매0),
⑧멱등성(200→409), ⑨체크포인터 생존성(서버 재시작 후 대화 유지).

### FastAPI — 멀티턴/재고 질문 버그 수정 (연쇄적으로 발견된 것 순서대로)
1. **[분류] `classify_node`가 히스토리를 안 봄** — 지시어("그 중", "그거") 해석
   실패로 완전히 무관한 상품 응답. `INTENT_SYSTEM_PROMPT`+`classify_node`에
   `MessagesPlaceholder("history")` 주입.
2. **[가격 비교] LLM 자유생성이 히스토리 속 가격을 잘못 비교** — "그 중 제일
   싼 건?"에서 실제 최저가가 아닌 상품을 답함. `_detect_price_superlative`
   `_match_history_products`로 파이썬 결정적 비교 경로 신설(LLM 미호출).
3. **[가드 형식] rag_hits가 guard 기대 형식(ChromaDB `metadata.*`)이 아니라
   정상 답변도 환각으로 오판** — `_to_rag_hit()` 변환 헬퍼로 형식 통일.
4. **[매칭 오탐] "35,000원" 이 "135,000원" 의 부분 문자열이라 잘못 매칭** —
   `_price_mentioned_exactly()` 정규식 lookbehind 로 숫자 경계 보정.
5. **[재고 필터 충돌] 품절 상품도 정확히 "품절"이라 답해야 하는데 사전
   `stock>0` 필터가 후보 자체를 제거** — 단일 참조 경로만 필터 제거.
6. **[Function Calling 전환]** 텍스트/토큰 매칭이 카탈로그 공유 단어("오버핏"이
   두 상품에 공통), 빈 히스토리 등 계속 새 방식으로 깨짐 → LLM이 실제 카탈로그
   목록을 보고 직접 판단하는 구조화 출력(`schemas/product_query_schema.py`
   `ProductAttributeQuery`)으로 교체. 가격 비교(②)는 결정성이 중요해 파이썬
   로직 유지, 참조 상품 탐색만 교체.
7. **[근본 원인] 게스트 히스토리가 서버로 아예 전송 안 됨** — `chat.py`는
   "게스트는 클라이언트가 history 전송"으로 설계돼 있었는데 `chatbot.js`
   `send()`가 로컬 저장(`pushHistory`)만 하고 요청 payload에 history를 실은
   적이 없었음. 그동안의 멀티턴 버그 다수의 근본 원인. `payload.history`에
   이번 질문 push 전 히스토리를 캡처해 전송하도록 수정.
8. **[SOLD_OUT 노출]** 검색 인덱스는 품절 상품도 포함(의도된 설계, metadata로
   필터링 예정이었으나 필터 자체가 누락)했는데 검색 결과 필터링이 안 돼
   스토어프론트엔 없는 품절 상품을 챗봇이 안내 — `search_and_rerank()` 최종
   후보 단계에 `stock<=0` 필터 추가(인덱싱은 원래 설계대로 유지).
9. **[마크다운 노출]** 프롬프트로 `**굵게**` 금지해도 LLM이 종종 사용 —
   `guard_node`(전 인텐트 답변 합류점)에서 정규식으로 강제 제거.
10. **[재인사 반복]** 매 턴 "안녕하세요"로 시작 — `SYSTEM_PROMPT`에 "대화
    진행 중이면 재인사 금지" 추가.
11. **[DeepSeek thinking mode]** `deepseek-v4-flash` 가 기본 thinking mode 라
    `.with_structured_output()`(강제 tool_choice)과 충돌, 인텐트 분류가 매번
    400 에러로 실패(정확도 12%까지 하락). `extra_body={"thinking":
    {"type": "disabled"}}` 로 해결(DeepSeek API 알려진 제약).
12. **[모델명 오류]** `gemini-3.1-flash` 가 실제 존재하지 않는 이름(정확한
    이름은 `gemini-3.1-flash-lite`) — `model_factory.py` 4곳 수정.

### RAGAs CI 게이트 버그 2건 (②를 실측 검증하다 발견)
- **ragas>=0.4 의 상위 라이브러리 버그** — 이미 삭제된
  `langchain_community.chat_models.vertexai` 경로 참조로 import 자체가 실패.
  `ragas==0.3.9` + `langchain-community<0.4` 로 버전 고정(GitHub issue #2745).
- **NaN 이 게이트를 무력화하는 자체 코드 버그** — `actual < minimum` 비교가
  `actual`이 NaN이면 파이썬에서 항상 False라, 임계값을 아무리 올려도 조용히
  통과해버림(정상 케이스와 실패 케이스 둘 다 거짓 통과였을 가능성). NaN 명시적
  실패 처리로 수정, `evaluate()` 호출에 `llm=`/`embeddings=` 명시(NaN 근본 원인
  — 미지정 시 RAGAs 가 내부 기본값으로 조용히 실패).

### Spring Boot 수정 2건
- **결제금액 "011원" 표시 버그** — `#numbers.formatInteger(값, 3, 'COMMA')`
  의 두 번째 인자(최소 정수 자릿수)가 3으로 잘못돼 3자리 미만 금액에 0 패딩.
  `1`로 정정(3개 템플릿 6곳: list/detail/complete.html).
- **로그아웃 후 챗봇 대화 잔존** — 로그아웃 리다이렉트에 이미 실려오는
  `?logout=true` 신호로 `chatbot.js`에서 세션 저장소 초기화(서버 변경 없음).

### 검증
- pytest **154개 중 152 통과**(MCP 2개는 기존과 동일한 알려진 환경 제약).
  이번 라운드 신규: test_semantic_price_superlative.py 10개(전면 재작성),
  test_guard_markdown_strip.py 2개(신규), test_rag_enhancement.py +2개(품절 필터).
- 매 수정마다 실제 재현 시나리오를 회귀 테스트로 고정 — 특히 `_price_mentioned_exactly`
  정규식 버그는 회귀 테스트 자체가 실패하는 과정에서 제가 만든 수정의 오류를
  스스로 잡아낸 사례(부분 문자열 오탐 2차 발견).

### 문서
- PRESENTATION_SCRIPT.md: 11절을 "배포 후 실측 결과"로 전면 교체(①③), RAGAs
  문단에 ④ 실측 4구성표 반영, CI 질문수 정정(6→18), 테스트 현황·총계 갱신.
- PHASE3_BENCHMARK.md: 전 절 실측 반영(CPQ/QPD 단가 자체 오류 정정 포함),
  7절(④하이브리드 ablation + ⑤개인화 A/B) 신설, 6절(알려진 한계) 실측 기반 갱신.
- ROADMAP.md: "PHASE 3 이후 — 실측(①~⑨) 완료 + 멀티턴 근본 버그 수정" 섹션 신설.
- MEASUREMENT_GUIDE.md: 단가 예시·평가셋 파일명 오류 정정, 완료 상태 안내 추가.

## 27. [챗봇 답변 버그 수정 2건] + [미연동 기능 3건 실제 연동] + 동적 모델 선택 활성화

실사용 중 발견된 챗봇 답변 버그(조사 오류, 가격 필터링 0건)를 수정하고, 백엔드
코드는 이미 완성돼 있었지만 프론트엔드/운영 설정이 빠져 있던 기능 3건(STT/TTS,
환불 관리자 알림, CLIP 이미지 검색)을 실제로 연동했다. Spring Boot 쪽 버그
2건(회원가입 500 에러, 마이페이지 헤더 불일치)도 같이 수정.

### FastAPI — 챗봇 답변 버그 수정
1. **[조사 오류] 재고/품절 답변에 "은" 하드코딩** — "니트 재고 있어?" →
   "니트은 현재 재고 30개 있어요" 같은 부자연스러운 조사. `graph/nodes.py`에
   `_josa_eun_neun()` 추가(받침 유무 판별, 한글 완성형 범위 밖 문자는 "은(는)"
   폴백), `_resolve_product_attribute_query` 응답 조립부(재고/품절 분기) 2곳에
   적용.
2. **[가격 필터 0건] "5만원 이하 상의" 같은 질문이 실제 매칭 상품이 있어도
   0건 반환** — 재현 확인 결과, LLM이 `category`와 `keywords`에 같은 단어
   ("상의")를 중복 추출하면 `PRODUCT_NAME LIKE '%상의%'` 조건이 끼어드는데
   상품명엔 카테고리 단어가 안 들어가서(예: "린넨 반팔 셔츠") AND 결합 시
   카테고리+가격 조건에 맞는 상품이 있어도 전부 걸러짐. `database/oracle_db.py`
   `build_structured_query()`에서 category와 정확히 같은 keyword를 LIKE 조건
   생성 전에 제거(부분/포함 매칭은 다른 키워드를 잘못 지울 위험이 있어 정확
   일치만 적용). `graph/nodes.py`(structured_node)·`pipeline/structured_handler.py`
   양쪽이 이 함수를 공유해 한 곳 수정으로 둘 다 해결됨.
3. **[멀티턴 조건 이월] RAG 답변이 이전 턴의 가격/카테고리 조건을 이번 질문에
   재적용** — "5만원 이하 상의" 질문 이후 조건 없이 "니트 재고 있어?"라고
   물어도 "5만원 이하라 없다"고 잘못 답함. `generate_rag_response`가 매번
   전체 대화 이력을 LLM에 넘기면서 "맥락을 이어서 답변하라"고만 지시했던 게
   원인. `services/rag_service.py` `SYSTEM_PROMPT`에 "지시어 해석엔 이력을
   참고하되, 가격/카테고리 조건은 이번 질문과 상품 컨텍스트만 근거로 판단"
   규칙 추가(프롬프트 보강 — 결정적 차단은 아니며 재현 시 재점검 필요).

### Spring Boot — 회원가입/마이페이지 버그 수정
1. **[500 에러] 이메일 중복 가입 시 Whitelabel 500** — `MemberController.join()`에
   try-catch가 없어 `DataIntegrityViolationException`(`UK_MEMBER_EMAIL` 유니크
   제약 위반)이 그대로 전파. try-catch 추가해 `member/joinForm`으로 복귀 +
   에러 메시지 표시(`joinForm.html`에 표시 영역 추가). 입력값 유지(재입력 방지)는
   이번 범위 밖.
2. **[헤더 불일치] 마이페이지가 구버전 `layouts/header`/`layouts/footer` 사용** —
   상품 목록 페이지(`fragments/header`/`fragments/footer`)와 다른 레이아웃으로
   렌더링(검색창 스타일 없음, 세션 키 체계도 구버전). `fragments/header`/
   `fragments/footer`로 통일, body 상단 여백(구 fixed 헤더 보정용 60px 패딩,
   새 헤더는 sticky라 불필요)을 페이지 국소 오버라이드로 제거.

### 미연동 기능 3건 실제 연동
1. **STT/TTS** — 백엔드(`routers/voice.py` `POST /chat/voice`: STT→기존
   파이프라인→TTS를 한 번에 처리)는 완성돼 있었지만 챗봇 위젯에 마이크 UI
   자체가 없었음. `fragments/chatbot.html`에 마이크 버튼, `chatbot.js`에
   `MediaRecorder` 녹음(클릭 토글) → `/chat/voice` 전송 → 질문/답변 텍스트
   표시 + `audio_base64` TTS 자동재생 로직 추가.
2. **환불 신청 관리자 알림** — `request_refund()`(Human-in-the-loop)가
   사용자 승인 후 `logger.info()`만 남기고 관리자에겐 아무 알림도 안 갔음.
   `services/notification_service.py` 신규(표준 라이브러리 `smtplib`, 새
   의존성 없음)로 관리자 이메일 발송, `request_refund()` 승인 분기에서 호출
   (best-effort — 발송 실패해도 사용자 응답은 그대로 진행). 실제 ORDERS 상태
   변경은 여전히 Spring Boot 책임 영역으로 남김(DB 쓰기 없음, 기존 설계 유지).
3. **CLIP 이미지 검색 활성화** — 코드(`clip_service.py`, `rag_pipeline.py`)는
   이미 구현돼 있었지만 `CLIP_SERVING_ENABLED=false` + 전 상품 `IMAGE_URL
   NULL`이라 비활성 상태였음. 상품 이미지 22건 실제 등록(관리자 화면에서
   `IMAGE_URL` 입력) + 서버에 `torch`/`open_clip_torch` 설치(ARM aarch64,
   CPU 전용으로 정상 동작 확인, `cuda.is_available()=False`) +
   `scripts/index_products_image.py` 실행(22건 성공/1건 URL없음/0건 실패) +
   플래그 활성화로 실제 동작 확인(서버 로그: "SEMANTIC 멀티모달 후보: 텍스트
   10 + 이미지 10 → 병합 N건"). 코드 변경 없이 데이터 등록 + 운영 설정만으로
   해결.

### 설정 변경
- `DYNAMIC_MODEL_SELECTION=false→true` — 현재 `small_talk_node`(SIMPLE,
  인사/잡담) 하나만 이 정책에 연결돼 있어, 활성화 시 이 노드만 저비용 모델
  (`get_intent_llm`, 기본 gpt-5.4-mini)로 분기되고 나머지 5개 인텐트는
  플래그와 무관하게 기존 고품질 모델(`get_main_llm`) 그대로 유지.

### 검증
- pytest **171개 중 169 통과**(MCP 2개는 기존과 동일한 알려진 환경 제약).
  환불 human-in-the-loop 테스트(`test_human_in_the_loop.py` 7개, 알림 호출이
  추가된 `test_refund_resume_approve` 포함) 전부 통과 확인.
- `build_structured_query(category='상의', keywords=['상의'])` 직접 호출로
  키워드 중복 제거 전/후 SQL을 비교해 회귀 재현 확인.

### 문서 표 재동기화 (이번 항목과 무관한 기존 오차 발견 후 처리)
`ROADMAP.md`/`PRESENTATION_SCRIPT.md`의 "테스트 현황" 표가 154개로 표기돼
있었는데 실측은 171개였다(이번 작업이 원인 아님 — `pytest --collect-only`로
파일별 개수 전수 확인 결과 `scripts/test_structured_offline.py`(17개)가 표
작성 시점부터 누락돼 있었던 것). 두 문서 표 모두 해당 행 추가 + 총계 정정.

### 문서
- IMPROVEMENTS.md: 이번 항목(27번) 신설.
- ROADMAP.md: "PHASE 3 이후 — 챗봇 답변 버그 수정 + 미연동 기능 3건 연동" 섹션
  신설, 테스트 현황 표 재동기화(154→171).
