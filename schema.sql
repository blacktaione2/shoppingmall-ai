...
/
BEGIN EXECUTE IMMEDIATE 'DROP TABLE CHAT_SESSION CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN NULL; END;
/
...
--==============================================================================
-- 10-1. CHAT_SESSION — 대화방(Thread) 목록 (로그인 회원 전용, 다중 세션)
--
-- [AI 연계]
--   · 회원당 CHAT_TOKEN 은 1개(인증 전용)지만, 대화방은 회원당 여러 개 가질 수
--     있다. LangGraph 의 thread_id 로 SESSION_ID 를 쓴다(CHAT_TOKEN 아님).
--   · TITLE 은 대화방 첫 질문을 40자로 잘라 저장(LLM 요약 미사용).
--==============================================================================
CREATE TABLE CHAT_SESSION (
    SESSION_ID   VARCHAR2(36)   NOT NULL,  -- UUID
    MEMBER_ID    NUMBER         NOT NULL,
...
-- CHAT_SESSION
CREATE INDEX IX_CHAT_SESSION_MEMBER ON CHAT_SESSION (MEMBER_ID, UPDATED_AT DESC);
-- · 회원별 대화방 목록(최근 활동순) 조회
